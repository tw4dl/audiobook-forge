...
pub(crate) fn write_suggestions(path: &Path, report: &PreflightReport) -> Result<()> {
    ...
    suggestions.sort();
    suggestions.dedup();
    let mut text = String::from("# Generated by kokoro-book preflight. Review before use.\n");
...
mod tests {
    ...
    fn book(text: &str) -> CanonicalBook {
        ...
        CanonicalBook {
            ...
            source: SourceDocument {
                path: std::env::temp_dir().join("kokoro-book-preflight-test.txt"),
    ...
    fn batch_reports_all_unknown_tokens_and_deduplicates() {
        let path = std::env::temp_dir().join("kokoro-book-preflight-test.txt");
